...
from app.core.privacy import anonymize_text
from app.core.shortcuts import shortcuts_manager
...
class NLPEngine:
    ...
    def learn(self, text: str):
        ...
        try:
            ...
            masked = anonymize_text(text[:64])
            logger.info(f"[NLP] Learned from input (masked): {masked}")
        except Exception as e:
            logger.warning(f"Error learning from text: {e}")
    ...
    def _prefix_impl(
        self, prefix: str, max_results: int, *, trie_only: bool = False, user_id: str = "default"
    ) -> List[Dict]:
        if not prefix:
            return []
        ...
        # 0. Kullanıcı kısaltmaları (slm -> selam)
        shortcut_full = shortcuts_manager.get_shortcut(user_id, prefix_lower)
        if shortcut_full:
            results.append(
                {
                    "word": shortcut_full,
                    "count": 10_000,  # Çok güçlü prior
                    "source": "shortcut",
                }
            )
            seen.add(shortcut_full)
            ...
            for word in user_matches_words:
                if word not in seen:
                    results.append(
                        {
                            "word": word,
                            "count": self.user_dict.get_frequency(word) * 200,
                            "source": "user",
                        }
                    )
                    seen.add(word)
        ...
        if self.trie_engine:
            trie_results = self.trie_engine.search(
                prefix_lower, max_results=max_results * 2
            )
