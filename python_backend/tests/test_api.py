...
def test_edge_cases():
    ...
    long_text = "a" * 1000
    headers = {"X-API-Key": "texthelper-secret-key-2024"}
    response = client.post("/api/v1/predict", json={"text": long_text}, headers=headers)
    assert response.status_code == 200
    
    special_chars = "!!!@@@###$$$%%%^^^&&&"
    response = client.post("/api/v1/predict", json={"text": special_chars}, headers=headers)
    assert response.status_code == 200
    
    injection_attempt = "<script>alert('xss')</script>"
    response = client.post("/api/v1/predict", json={"text": injection_attempt}, headers=headers)
    assert response.status_code == 200
...
def test_learn_interaction():
    """Test /learn interaction endpoint with selected suggestion."""
    payload = {
        "text": "merhaba",
        "selected_suggestion": "merhabalar",
        "user_id": "test-user",
    }
    headers = {"X-API-Key": "texthelper-secret-key-2024"}
    response = client.post("/api/v1/learn", json=payload, headers=headers)
    assert response.status_code == 200
    data = response.json()
